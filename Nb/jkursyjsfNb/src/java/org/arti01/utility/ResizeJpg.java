...
public class ResizeJpg {
    ...
    public byte[] zrobB(int dlugDocel, int wysDocel, byte[] lf) {
        ...
        try {
            ...
            BufferedImage var5 = ImageIO.read(buffin);
            //BufferedImage var5 = ImageIO.read(new File(var3));
            BufferedImage var10 = new BufferedImage(dlugDocel, wysDocel, 1);
            Graphics2D var11 = var10.createGraphics();
            BufferedImage varD;

            //jesli rozmiar foty mniejszy niż skalowanie, to nie powiekszamy
            if (wysDocel >=var5.getHeight() || dlugDocel >= var5.getWidth()) {
                return lf;
            }

            wysokosc = wysDocel;
            if (wysDocel < var5.getHeight()) {
                dlugosc = var5.getWidth() * wysokosc / var5.getHeight();
                wysokosc = 0;
            } else {
                dlugosc = var5.getWidth();
                wysokosc = (wysDocel - var5.getHeight()) / 2;
            }
            logger.info("waskie" + dlugosc + "--" + var5.getWidth() + "--" + wysokosc + "--" + var5.getHeight());
            AffineTransform var14 = AffineTransform.getTranslateInstance((dlugDocel - dlugosc) / 2, wysokosc);
            var11.setTransform(var14);
            logger.info(dlugosc + "++" + wysDocel);
            if (wysDocel < var5.getHeight()) {
                varD = new BufferedImage(dlugosc, wysDocel, 1);
            } else {
                varD = new BufferedImage(var5.getWidth(), var5.getHeight(), 1);
            }
