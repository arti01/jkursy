...
<s:form method="post" namespace="/admin" action="users!dodaj.action">
	<s:actionerror />
	<s:textfield label="login" name="user.username" size="70"
		readonly="zmien" />
	<s:textfield label="email" name="user.email" value="%{user.email}"
		size="70" readonly="%{zmianaHasla}" />
	<s:if test="%{(!zmien) or (zmianaHasla)}">
		<s:password label="haslo" name="user.userpass"
			value="%{user.userpass}" size="70" />
	...
	<s:textfield label="imię i nazwisko" name="user.imieNazwisko" size="70"
		readonly="%{zmianaHasla}" />
	<s:textarea label="opis (ważne dla wykładowcy)" name="user.opis"
		rows="10" cols="70" readonly="%{zmianaHasla}" />
	<s:textfield label="ulica" name="user.ulica" size="70"
		readonly="%{zmianaHasla}" />
	<s:textfield label="miasto" name="user.miasto" size="70"
		readonly="%{zmianaHasla}" />
	<s:textfield label="nip" name="user.nip" size="70"
		readonly="%{zmianaHasla}" />
	<s:textfield label="telefon" name="user.tel1" size="70"
		readonly="%{zmianaHasla}" />
	<s:textfield label="data ostatniej modyfikacji" key="user.dataZmiany"
		value="%{getText('format.daty',{user.dataZmiany})}" readonly="true" />
